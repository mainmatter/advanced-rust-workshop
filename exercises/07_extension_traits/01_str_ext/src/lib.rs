//! # Exercise
//!
//! Two ways to spell the same thing, and the point of the exercise is the difference between them.
//!
//! **One: an extension trait.** You own the trait, so you may implement it for `str`, which you do
//! not own:
//!
//! ```text
//! pub trait StrExt {
...
//! ```
//!
//! Implement it for `str`, not for `&str` or `String`: auto-deref then makes it work for all three.
//! Both methods just call the parser you already have.
//!
//! **Two: the trait the standard library already has.** `FromStr` exists precisely for this, and
//! implementing it gives you `"users".parse::<Bucket>()` with no trait of your own and nothing for
//! your callers to import:
//!
//! ```text
...
//! ```
//!
//! Implement both, for both types. Then look at the two test cases and decide which one you would
...
mod tests {
    ...
    fn the_extension_trait_reads_well() {
        ...
        assert_eq!(
            store
                .get(&"users".to_bucket().unwrap(), &"42".to_key().unwrap())
                .map(Value::as_str),
            Some("Alice")
        );
    }
    ...
    #[test]
    fn the_extension_trait_still_validates() {
        assert_eq!(
            "user 42".to_key().err(),
        ...
        assert_eq!("".to_bucket().err(), Some(NameError::Empty));
    }

    #[test]
    fn from_str_reads_just_as_well_and_needs_no_import() {
        let mut store = Store::new();
    ...
    #[test]
    fn it_works_through_a_string_and_a_reference() {
        let owned = String::from("users");
        let borrowed: &str = &owned;

        assert!(owned.to_bucket().is_ok());
        assert!(borrowed.to_bucket().is_ok());
    }
}
