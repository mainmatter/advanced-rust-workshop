//! # Exercise
//!
//! Nothing to write here. Read the test, run `wr`, and move on.
//!
//! `Store::export` renders one format, and the format is baked into the method. The next request is
//! always CSV, or JSON, or whatever the reporting team uses this quarter.
//!
//! Rust offers two ways to be polymorphic over "some format", and they are not interchangeable:
//!
//! ```text
//! fn export_with<F: Format>(&self, format: F) -> String     // one copy per format, decided at compile time
//! fn export_into(&self, format: &mut dyn Format) -> String  // one copy total, decided at run time
//! ```
//!
//! The next exercise builds both, and the interesting part is what the trait has to look like for
//! the second one to be legal at all.
