...
//! }
//!
//! Scoped::key(&self, raw: &str) -> Result<ScopedKey<'brand>, NameError>
//! Scoped::insert(&mut self, bucket: Bucket, key: ScopedKey<'brand>, value: Value) -> Option<Value>
//! Scoped::get(&self, bucket: &Bucket, key: &ScopedKey<'brand>) -> Option<&Value>
//!
...
//! ```
//!
//! Two details carry the whole trick, and neither is guessable:
//!
//! **`for<'brand>`** means the closure must work for *every* lifetime, so it cannot be written to
...
//!
//! ```compile_fail
//! use phantom_branded::{Bucket, scope};
//!
...
#[cfg(test)]
mod tests {
    use crate::{Bucket, Value, scope};

    #[test]
    fn a_branded_key_works_with_its_own_store() {
        let users = Bucket::parse("users").unwrap();

        let found = scope(|mut store| {
            let id = store.key("42").unwrap();
            store.insert(users.clone(), id, Value::new("Alice"));
    ...
    #[test]
    fn a_branded_key_still_validates() {
        let rejected = scope(|store| store.key("user 42").is_err());

        assert!(rejected);
    }

    #[test]
    fn each_scope_starts_empty() {
        let users = Bucket::parse("users").unwrap();
        ...
        let found = scope(|store| {
            let id = store.key("42").unwrap();
            store.get(&users, &id).is_some()
        });
