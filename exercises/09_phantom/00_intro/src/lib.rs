...
//!
//! You have been using `PhantomData` since chapter 6 without asking what it does. It is not only a
//! way to silence "unused type parameter": it is how a type opts into a relationship its fields do
...
//!
//! assert_send(Handle { id: 1, _marker: PhantomData });
//! ```
//!
//! And the zero cost is real, not a figure of speech:
//!
//! ```
//! use phantom_intro::{ReadOnly, ReadWrite, Transaction};
//! use std::mem::size_of;
//!
//! assert_eq!(
...
//! ```
//!
//! This chapter is the tail of the day: the same idea, pushed until it stops being obvious.
...
#[cfg(test)]
mod tests {
    use crate::{ReadOnly, ReadWrite, Store, Transaction};
    use std::mem::size_of;
