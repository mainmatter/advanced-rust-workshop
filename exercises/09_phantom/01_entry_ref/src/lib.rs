//! # Exercise
//!
//! A handle that remembers *where* a value lives, without borrowing the store to hold it:
//!
//! ```text
...
//! existed when you asked.
//!
//! Look closely at the struct: it owns a `Bucket` and a `Key`, both `'static`, and borrows nothing.
//! Without the `PhantomData` it could outlive the store, be sent to another thread, and be read
//! against a store that has since deleted the entry. The compiler would have no objection, because
//! nothing in the data says otherwise.
//!
//! `PhantomData<&'store Store>` says otherwise. It makes the handle behave exactly like a shared
//! borrow of the store, which is what it morally is, so all the usual rules apply for free:
//!
//! ```compile_fail
//! use phantom_entry_ref::{Bucket, Key, Store, Value};
//!
...
//!     let mut store = Store::new();
//!     store.insert(users.clone(), id.clone(), Value::new("Alice"));
//!     store.entry_ref(&users, &id).unwrap()
//! };
//!
//! let _ = escaped;
//! ```
...
mod tests {
    ...
    #[test]
    fn a_handle_reads_the_value_it_points_at() {
        let mut store = Store::new();
        let users = Bucket::parse("users").unwrap();
        let id = Key::parse("42").unwrap();
        store.insert(users.clone(), id.clone(), Value::new("Alice"));
    ...
    #[test]
    fn there_is_no_handle_to_a_missing_entry() {
        let store = Store::new();
        let users = Bucket::parse("users").unwrap();
        let id = Key::parse("42").unwrap();

        assert!(store.entry_ref(&users, &id).is_none());
    }

    #[test]
    fn several_handles_can_coexist() {
        let mut store = Store::new();
        let users = Bucket::parse("users").unwrap();
        let alice = Key::parse("42").unwrap();
        let bob = Key::parse("43").unwrap();
        store.insert(users.clone(), alice.clone(), Value::new("Alice"));
        store.insert(users.clone(), bob.clone(), Value::new("Bob"));

        let first = store.entry_ref(&users, &alice).unwrap();
        let second = store.entry_ref(&users, &bob).unwrap();

        assert_eq!(store.read(&first).map(Value::as_str), Some("Alice"));
        assert_eq!(store.read(&second).map(Value::as_str), Some("Bob"));
    }
}
